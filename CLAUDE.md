...
- **Transactional email** uses **react-email** (`emails/*.tsx`). The brand palette is the `globals.css` OKLCH tokens hand-converted to email-safe hex in `emails/brand.ts` (email clients support neither OKLCH nor CSS variables — keep this in lockstep with `globals.css`). No sending provider is wired yet; `render()` from `@react-email/components` produces the HTML. Social icons are self-hosted PNGs in `public/email/` referenced as **absolute** URLs via `NEXT_PUBLIC_APP_URL` (mail clients can't load relative paths). Preview with `npm run email:dev`.
- **Path alias**: `@/*` → `src/*`.
...
- Required env vars (see `.env.example`): `DATABASE_URL`, `AUTH_SECRET`, `NEXT_PUBLIC_APP_URL` (production: `https://volunteer.fairfood.org.nz`).
