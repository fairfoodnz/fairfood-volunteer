...
- **Programme visibility** (`Program.visibility`, enum `ProgramVisibility`, replaced the old `active` boolean): `PUBLIC` (listed + bookable) → `UNLISTED` (hidden from every listing and the sitemap, reachable and bookable by direct link) → `PRIVATE` (no public surface; coordinators roster volunteers from `/admin` with "Assign volunteer") → `ARCHIVED` (private *and* dropped from the admin scheduling pickers). Never compare the enum inline — `isListedPublicly` / `isLinkVisible` / `isSchedulable` in `src/lib/programs.ts` are the single source of the rule, and every new surface must go through them. Anything non-`PUBLIC` also renders with `NOINDEX` and emits no schema.org JSON-LD. `/shifts/[id]` stays reachable for a volunteer already booked on a hidden shift (their dashboard, confirmation email and calendar invite all link to it) and for admins; everyone else gets a 404.
